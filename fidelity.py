...
def train_graph_model(task: str,
    ...
    # print(graph.edges(form='eid')[('claim', 'discusses_inv', 'tweet')])

    '''For fadelity+'''
    # for item  in args.inputs:

    # '''For fadelity-'''
    inputs = ['tweet_text', 'tweet_lang', 'claim_embedding','claim_reviewer', 'image', 'hashtag', 'user', 'reply_text', 'reply_lang',
    ...
    #End here
        ...
        if item[0] == 'e':
            item = etypes_dict[int(item[1:])-1]
            graph.remove_edges(torch.arange(graph.number_of_edges(etype=item[0]), dtype=torch.int32), etype=item[0])
            graph.remove_edges(torch.arange(graph.number_of_edges(etype=item[1]), dtype=torch.int32), etype=item[1])

        elif item == 'tweet_text':
            graph.nodes['tweet'].data['feat'][:, :768] = torch.zeros_like(graph.nodes['tweet'].data['feat'][:,:768])

        elif item == 'tweet_lang':
            graph.nodes['tweet'].data['feat'][:, 768:810] = torch.zeros_like(graph.nodes['tweet'].data['feat'][:, 768:810])

        elif item == 'claim_embedding':
            graph.nodes['claim'].data['feat'][:, :768] = torch.zeros_like(graph.nodes['claim'].data['feat'][:, :768])

        elif item == 'claim_reviewer':
            graph.nodes['claim'].data['feat'][:, 768:] = torch.zeros_like(graph.nodes['claim'].data['feat'][:, 768:])

        elif item == 'image':
            graph.nodes['image'].data['feat'] = torch.zeros_like(graph.nodes['image'].data['feat'])

        elif item == 'hashtag':
            graph.nodes['hashtag'].data['feat'] = torch.zeros_like(graph.nodes['hashtag'].data['feat'])

        elif item == 'user':
            graph.nodes['user'].data['feat'] = torch.zeros_like(graph.nodes['user'].data['feat'])

        elif item == 'reply_text':
            graph.nodes['reply'].data['feat'][:,:768] = torch.zeros_like(graph.nodes['reply'].data['feat'][:,:768])

        elif item == 'reply_lang':
            graph.nodes['reply'].data['feat'][:,768:829] = torch.zeros_like(graph.nodes['reply'].data['feat'][:,768:829])
...
results = train_graph_model(task='tweet', size='small', num_epochs=100, random_split=False)
print(results)
file = 'result/results_fidelity'
