...
def train_graph_model(task: str,
    ...
    edge_masks = {}
    i = 0
    explainer = HeteroGNNExplainer(model, num_hops=1, num_epochs=100).to(device)
    ...
    for etype_pair in etypes_dict:
        importance[etype_pair[0]] = (torch.mean(edge_masks[etype_pair[0]]/i) + torch.mean(edge_masks[etype_pair[1]]/i))/2

        # if imp.isnan():
        #     importance[etype_pair] = torch.tensor(0.)
        # else:
