...
from models.HeteroGNNExplainer_ce import HeteroGNNExplainer
...
def train_graph_model(task: str,
    ...
    for epoch in epoch_pbar:
        ...
        for _, _, blocks in train_dataloader:
            ...
            '''dont forgot to change model dim = 2'''
    ...
    edge_masks = {}
    i = 0
    explainer = HeteroGNNExplainer(model, num_hops=1, num_epochs=1).to(device)
    for _, _, blocks in train_dataloader:
        blocks = [block.to(device) for block in blocks]
        input_feats = {n: feat.float() for n, feat in blocks[0].srcdata['feat'].items()}
        feat_mask, edge_mask = explainer.explain_graph(blocks, input_feats)
        ...
        for canonical_etype, canonical_etype_mask in edge_mask.items():
            if i == 0:
                edge_masks[canonical_etype] = torch.mean(edge_mask[canonical_etype])
            else:
                edge_masks[canonical_etype] += torch.mean(edge_mask[canonical_etype])
        i += 1
    ...
    importance['tweet_text'] = torch.mean(feat_masks['tweet'][:768])
    importance['tweet_lang'] = torch.mean(feat_masks['tweet'][768:810])
    # importance['tweet_num_retweets'] = torch.mean(feat_masks['tweet'][-3])
    # importance['tweet_num_replies'] = torch.mean(feat_masks['tweet'][-2])
    # importance['tweet_num_quote_tweets'] = torch.mean(feat_masks['tweet'][-1])

    importance['claim_embedding'] = torch.mean(feat_masks['claim'][:768])
    importance['claim_reviewer'] = torch.mean(feat_masks['claim'][768:])

    importance['image'] = torch.mean(feat_masks['image'])
    importance['hashtag'] = torch.mean(feat_masks['hashtag'])
    importance['user'] = torch.mean(feat_masks['user']) #user has many features

    importance['reply_text'] = torch.mean(feat_masks['reply'][:768])
    importance['reply_lang'] = torch.mean(feat_masks['reply'][768:829])
    # importance['reply_num_retweets'] = torch.mean(feat_masks['reply'][-3])
    # importance['reply_num_replies'] = torch.mean(feat_masks['reply'][-2])
    # importance['reply_num_quote_tweets'] = torch.mean(feat_masks['reply'][-1])

    for canonical_etype in edge_mask:

        importance[canonical_etype] = torch.mean(edge_masks[canonical_etype])

    print(importance)

    return results, importance

results, importance = train_graph_model(task='tweet', size='small', num_epochs=1, random_split=True)
file = 'classification/log.txt'
