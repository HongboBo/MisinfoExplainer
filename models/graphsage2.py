...
class HeteroGraphSAGE(nn.Module):
    ...
    def forward(self, blocks, h_dict: dict, eweight=None, etype=None) -> dict:
        ...
        if eweight is None:
            ...
            for c_etype in blocks[0].canonical_etypes:
                if c_etype == etype:
                    d[c_etype] = {'edge_weight': self.w(blocks[0][c_etype], eweight)}
                else:
                    d[c_etype]= {'edge_weight': None}
            h_dict = self.conv1(blocks[0], h_dict, mod_kwargs=d)
